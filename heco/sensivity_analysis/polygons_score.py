...
=============================================================
Computes original configuration and perturbed configurations results.
-> Input file contains multitemporal GeoJSON polygons of the observed oil slick (O) and the simulated dispersion result (S).
...
Usage:
    ...
    from polygons_score import compute_metrics

    obs_path = Path("observed.geojson")
    sim_path = Path("simulated.geojson")

    # obs_idx and sim_idx select which feature (polygon) to compare from each
    # multitemporal GeoJSON file. For example, 0 means the first timestamp,
    # 1 means the second timestamp, and so on.
...
def compute_metrics(obs_path: Path, sim_path: Path, obs_idx: int = 0, sim_idx: int = 0, projected_crs: str = DEFAULT_PROJECTED_CRS) -> dict:
    """
    ...
    obs_idx : int, default 0
        Index of the feature to use from the observed GeoJSON file.
    sim_idx : int, default 0
        Index of the feature to use from the simulated GeoJSON file.
    ...
    # Use the requested feature from each layer
    if obs_idx < 0 or obs_idx >= len(gdf_O):
        raise IndexError(f"Observed GeoJSON index {obs_idx} is out of range for {obs_path}")
    if sim_idx < 0 or sim_idx >= len(gdf_S):
        raise IndexError(f"Simulated GeoJSON index {sim_idx} is out of range for {sim_path}")

    geom_O = gdf_O.geometry.iloc[obs_idx]
    geom_S = gdf_S.geometry.iloc[sim_idx]
...
if __name__ == "__main__":
    metrics = compute_metrics(OBS_PATH, SIM_PATH, obs_idx=0, sim_idx=0, projected_crs=DEFAULT_PROJECTED_CRS)
